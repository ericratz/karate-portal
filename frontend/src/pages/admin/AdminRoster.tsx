// Admin Roster — React port of admin/students.php. Same four type-grouped
// cards and live filters as the instructor Roster, plus the admin extras:
// search also matches email/phone, a Registration (paid) column, the waiver
// check links to the waiver document, and the + New Participant button.
// Rows hide via inline display and the count badges track visible rows —
// the shared specs assert both.

import { useEffect, useMemo, useState } from 'react';
import { Link, useSearchParams } from 'react-router-dom';
import { apiGet, ApiError } from '../../api/client';
import type { AdminRosterData, AdminRosterStudent } from '../../api/types';
...
function rowMatches(s: AdminRosterStudent, f: Filters): boolean {
  if (f.q) {
    const q = f.q.toLowerCase().trim();
    const name = `${s.last_name} ${s.first_name} ${s.first_name} ${s.last_name}`.toLowerCase();
    const email = (s.email ?? '').toLowerCase();
    const qDigits = q.replace(/\D/g, '');
    const phone = (s.phone ?? '').replace(/\D/g, '');
    const match =
      name.includes(q) || email.includes(q) || (qDigits.length > 0 && phone.includes(qDigits));
          ...
          {rows.map((s) => {
            ...
            return (
              <tr
                ...
                data-phone={(s.phone ?? '').replace(/\D/g, '')}
                data-status={s.active ? 'active' : 'inactive'}
                data-login={s.has_login ? 'yes' : 'no'}
                data-rank={s.kyu_dan ?? ''}
                data-waiver={s.injury_waiver ? 'yes' : 'no'}
                data-last-attended={s.last_attended ?? ''}
              ...
              >
                ...
                <td>
                  {s.injury_waiver ? (
                    <a href={`waiver_view.php?student_id=${s.id}`} className="text-success text-decoration-none">✓</a>
                  ) : (
                ...
                <td>{s.last_attended ? fmtDate(s.last_attended) : 'Never'}</td>
                <td>
                  {s.active ? (
...
export default function AdminRoster() {
  ...
  const [data, setData] = useState<AdminRosterData | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [filters, setFilters] = useState<Filters>(emptyFilters);

  useEffect(() => {
    apiGet<AdminRosterData>('/admin/roster.php')
      .then(setData)
  ...
  const sections: [string, string, AdminRosterStudent[]][] = [
  ...
  return (
    <>
      ...
      <div className="d-flex gap-2 align-items-center mb-4 flex-wrap">
        ...
        <Link
          to="/admin/roster?sort=first_name"
          className={`btn btn-sm btn-filter ${sort === 'first_name' ? 'active' : ''}`}
        >
          First Name
        </Link>
        <Link
          to="/admin/roster?sort=last_name"
