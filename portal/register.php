...
if ($action === 'back1') {
    ...
    unset($_SESSION['reg'], $_SESSION['reg_matches'], $_SESSION['reg_selection']);

} elseif ($action === 'back2') {
    // Back to match screen — keep form data, clear selection
    unset($_SESSION['reg_selection']);
    ...
    if (!$first || !$last || !$dob || !$email || !$username || !$password) {
        ...
        if ($chk->fetch()) {
            ...
            // Store in session only — nothing written to DB yet
            $_SESSION['reg'] = [
                'first_name'    => $first,
                'last_name'     => $last,
                'date_of_birth' => $dob,
            ...
            $_SESSION['reg_matches'] = find_matches($first, $last, $dob, $email);
            // No matches → automatically select "new student" and skip to confirm
            if (empty($_SESSION['reg_matches'])) {
                $_SESSION['reg_selection'] = ['type' => 'new', 'student_id' => null];
            }
        }
    }
    ...
    $sel = $_SESSION['reg_selection']  ?? null;
    ...
    if (!$reg || !$sel) {
        ...
        try {
            ...
            // Stamp last_login — attempt_login() isn't called during registration
            db()->prepare('UPDATE users SET last_login = NOW() WHERE id = ?')
                 ->execute([$user_id]);

            db()->commit();

            // 4. Log user in immediately — full access right away
            session_regenerate_id(true);
            $_SESSION['user_id']  = $user_id;
            $_SESSION['role']     = $stype;
            $_SESSION['username'] = $reg['username'];
            audit('register', 'user', $user_id, "type=$alert_type");
            ...
            db()->rollBack();
            log_event('error', 'auth', 'Registration transaction failed', ['message' => $e->getMessage()]);
            $error = $e->getMessage();
        }
    }
}
...
if (!empty($_SESSION['reg'])) {
    if (!empty($_SESSION['reg_selection'])) {
        $step = 'confirm';
    } elseif (!empty($_SESSION['reg_matches'])) {
        $step = 'match';
    } else {
        // reg data present but no matches and no selection — auto-redirect to confirm as new
        $_SESSION['reg_selection'] = ['type' => 'new', 'student_id' => null];
        $step = 'confirm';
    }
} else {
...
$reg_data = $_SESSION['reg']           ?? [];
$matches  = $_SESSION['reg_matches']   ?? [];
$sel      = $_SESSION['reg_selection'] ?? [];
...
<head>
    ...
    <style nonce="<?= csp_nonce() ?>">
        ...
        /* Purple header with the black wordmark on it — see login.php. */
...
<div class="card shadow">
    ...
    <div class="card-header">
        ...
        <small class="brand-subtitle">
            <?php if ($step === 'form')   echo 'Create an Account';
    ...
    <!-- ── Step 3: Confirm ────────────────────────────────────────── -->
        ...
        <?php elseif (($sel['type'] ?? '') === 'not_listed'): ?>
            ...
            <p class="text-muted small mb-3">
                We'll create a new record from your registration info and flag it so
        ...
        <?php else: ?>
            ...
            <p class="text-muted small mb-3">
                <?= empty($matches)
                    ...
                    : 'We\'ll create a new student record from your registration info.' ?>
