...
if (!$signed && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$print_name || !$signature) {
        $error = 'Please print the name and provide a signature.';
    } elseif (!$agreed) {
...
$page_title = 'Liability Waiver — ' . htmlspecialchars($student['first_name'] . ' ' . $student['last_name']);
include __DIR__ . '/../includes/header.php';
?>

<div class="d-flex align-items-center gap-3 mb-4">
    <h4 class="mb-0">Liability Waiver — <?= htmlspecialchars($student['first_name'] . ' ' . $student['last_name']) ?></h4>
    <?php if ($signed): ?>
        <span class="badge bg-success ms-1">Signed <?= htmlspecialchars($student['injury_waiver_date']) ?></span>
    <?php endif; ?>
</div>
...
<div class="alert alert-success">
    This liability waiver was signed on <strong><?= htmlspecialchars($student['injury_waiver_date'] ?? 'file') ?></strong>.
    The waiver is on file — no further action is needed.
</div>
<?php if ($submission): ?>
<div class="card border-0 shadow-sm mb-4" style="max-width:760px">
    <div class="card-header bg-white fw-semibold">Submission on File</div>
    <div class="card-body">
        <dl class="row mb-0">
...
<!-- ── Waiver document (always visible) ── -->
    ...
    <div class="card border-0 shadow-sm mb-4">
        ...
        <div class="card-body">
            <form method="post" class="row g-3">
                ...
                <div class="col-md-6">
                    <label class="form-label">Print Full Name *</label>
                ...
                <div class="col-12">
                    <label class="form-label">
                        Signature *
                        <span class="text-muted small">(type full name — constitutes electronic signature)</span>
                    </label>
                ...
                <div class="col-12">
                    <button type="submit" class="btn btn-primary px-4">Submit Signed Waiver</button>
                </div>
            </form>
        </div>
    </div>
